...
if not args.isChild and (args.selectSys == "all" or args.selectSys == "combine"):
  ...
  for sys in (all_systematics if args.selectSys == "all" else ["combine"]):
    command = "python systematicsPlots_singleMu.py --selection=" + args.selection + (" --noData" if args.noData else "")\
               + (" --isChild")\
...
#    from nanoMET.samples.nanoTuples_Run2016_17Jul2018_postProcessed import *
    ...
    triggers                  = ["HLT_IsoMu24"] #"HLT_IsoMu24"
    METPtVar                 = "MET_T1_pt"
    METPhiVar                = "MET_T1_phi"
    JetCollection            = "Jet_pt"
    vetoEtaRegion            = (10.,10.)

    from nanoMET.samples.nanoTuples_UL18_nanoAODv9_postProcessed_mu_tuning import *
    from nanoMET.samples.nanoTuples_Run2018_ULnanoAODv9_postProcessed_mu_tuning import *

    data_sample              = SingleMuon_Run2018
    mc                       = [DY_LO_18, Top_18, diboson_18, rare_18, QCD_18, WJets_18]   
...
def addSys( selectionString , sys = None ):
    if   sys in jet_systematics: return selectionString.replace('Jet_pt', 'Jet_pt_' + sys).replace('MET_T1_pt', 'MET_T1_pt_' + sys)
    elif sys in met_systematics: return selectionString.replace('MET_T1_pt', 'MET_T1_pt' + sys)
...
for index, mode in enumerate(allModes):
    ...
    for sample in mc:
        ...
        if args.year == 2017:
            ...
            sample.read_variables += ["MET_T1_pt/F"]
            sample.read_variables += ["MET_T1_pt_%s/F"%s   for s in jet_systematics]
            sample.read_variables += ["MET_T1_pt_%s/F"%s   for s in met_systematics]
        sample.read_variables += ["MET_T1_significance_%s/F"%s    for s in jet_systematics]
        sample.read_variables += ["MET_T1_significance_%s/F"%s    for s in met_systematics]
    ...
    if args.year == 2016 or args.year == 2018:
        ...
        for sys in all_systematics:
            met_mc[sys] = Plot(
                ...
                attribute       = TreeVariable.fromString('MET_T1_pt/F') if sys not in jme_systematics else TreeVariable.fromString( "MET_T1_pt_%s/F" % sys ),
                binning         = [20,0,400],
    ...
    for sys in all_systematics:
        met_sig_mc[sys] = Plot(
            ...
            texY            = 'Number of Events',
            stack           = sys_stacks[sys],
            attribute       = TreeVariable.fromString('MET_T1_significance/F') if sys not in jme_systematics else TreeVariable.fromString( "MET_T1_significance_%s/F" % sys ),
            binning         = [20,0,100],
            selectionString = addSys(cutInterpreter.cutString(args.selection), sys),
            weight          = weightMC(sys)[0]#weight_,#lambda event, sample: int(1),
            )
